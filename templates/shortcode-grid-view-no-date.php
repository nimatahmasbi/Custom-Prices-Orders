...
<div class="cpo-grid-view-wrapper no-date-shortcode" style="--cpo-bg-logo: url('<?php echo esc_url($site_logo_url); ?>');">
    <?php if (($show_filters && !empty($categories)) || $last_updated_time) : ?>
        <div class="cpo-grid-view-filters">
            <?php if ($last_updated_time): ?><span class="last-update-display"><?php echo __('آخرین بروزرسانی:', 'cpo-full') . ' ' . date_i18n('Y/m/d H:i', strtotime(get_date_from_gmt($last_updated_time))); ?></span><?php endif; ?>
    ...
    <table class="cpo-grid-view-table">
        ...
        <tbody>
            ...
            <?php endforeach; else: ?>
                 <tr><td colspan="<?php echo $disable_base_price ? 6 : 7; ?>"><?php _e('محصولی یافت نشد.', 'cpo-full'); ?></td></tr>
            <?php endif; ?>
        </tbody>
         <tfoot>
             <tr>
    ...
    <div class="cpo-grid-view-footer">
        <button class="cpo-view-more-btn" data-page="1" data-shortcode-type="no_date"><?php _e('مشاهده بیشتر', 'cpo-full'); ?></button>
    </div>
    <?php endif; ?>
</div>